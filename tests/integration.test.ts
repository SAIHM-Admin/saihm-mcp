...
const responder: { for(method: string): unknown } = {
  for(method) {
    switch (method) {
      case "saihm_remember":
        return { cellId: "deadbeef".repeat(8), tier: "filecoin", kekVersion: 1, epoch: "493970", feeNcoti: "100000", signaturePrefix: "abcdef" };
      case "saihm_recall":
        return [
          { cellId: "aa".repeat(32), timestamp: "2026-05-09T00:00:00Z", tier: "filecoin", plaintext: "hello" },
          { cellId: "bb".repeat(32), timestamp: "2026-05-09T00:01:00Z", tier: "filecoin", plaintext: "world" },
        ];
      case "saihm_forget":
        return { success: true, cellId: "cc".repeat(32), destructionAnchor: "ee".repeat(32), epoch: "493971" };
      case "saihm_status":
        return {
          agentIdHashHex: "1234567890abcdef".repeat(4),
          prsScore: 42, prsLevel: "TIER_2",
          bfsiScore: 0.875,
          feeDiscountPct: 0.15,
          activeShardCount: 12,
      ...
      case "saihm_share":
        return { contractId: "ff".repeat(16), type: "temporary", granteeCount: 1, creationFeeNcoti: "50000", epoch: "493971" };
      case "saihm_revoke_share":
        return { revoked: true, epoch: "493972" };
      case "saihm_governance_propose":
        return { proposalId: "11".repeat(16), scope: "emission_param", paramKey: "block_reward", proposedValue: "1000", snapshotEpoch: "493970", proposerHash: "22".repeat(32) };
      case "saihm_governance_vote":
        return { proposalId: "11".repeat(16), voterHash: "33".repeat(32), approve: true, weight: "1000", castAtEpoch: "493971" };
      default:
...
async function main() {
  ...
  assert((calls[0].params as { content: string }).content === "test memory", "saihm_remember sent content");
  ...
  const shareParams = calls[4].params as { granteeIdHashesHex: string[]; expiryEpoch: string | null };
  ...
  const r7 = await client.governancePropose({ scope: "emission_param", paramKey: "block_reward", proposedValue: "1000" });
  ...
  // null expiry path
  await client.share([new Uint8Array([0x00])], ["s"], SharingContractType.PERMANENT, "readwrite", null);
  ...
  assert(FIELD_UNIVERSE.length === 280, `field universe size = ${FIELD_UNIVERSE.length} (expect 280)`);
  ...
  const badWindow = {
    ...goodTemplate,
    scope: { ...goodTemplate.scope, timeRange: { from: "2025-01-01T00:00:00Z", to: "2026-06-01T00:00:00Z" } },
  };
  assert(!validateBespokeTemplate(badWindow).valid, "rejects time window > 366d");

  const badFramework = { ...goodTemplate, framework: "not-a-framework" } as unknown as BespokeReportTemplate;
  ...
  const asSelf = await validateAuthSelf({
    path: "self", surface: "mcp", agentIdHash: "aa".repeat(32),
    signature: "sig", challenge: "ch", challengeIssuedAt: new Date().toISOString(),
  });
  assert(asSelf.ok, "auth/self mcp ok");

  const asOper = await validateAuthOperatorSelf({
    path: "operator-self", operatorIdHash: "ab".repeat(32),
    mldsaSignature: "sig", challenge: "ch",
  });
  assert(asOper.ok, "auth/operator-self ok");

  const asDown = await validateAuthOperatorForDownstream({
    path: "operator-for-downstream", operatorIdHash: "ab".repeat(32),
  ...
  const couplingFail = await validateAuthForKind("registry-attestation", {
    path: "operator-self", operatorIdHash: "ab".repeat(32), mldsaSignature: "sig", challenge: "ch",
  });
  assert(!couplingFail.ok, "kind-vs-auth coupling rejects mismatch");

  const replayFail = await validateAuthSelf({
    path: "self", surface: "web", walletIdHash: "aa".repeat(32),
  ...
  const r_gen = buildReportGenerated({
    kind: "bespoke", templateHash: "00".repeat(32),
    scope: goodTemplate.scope, format: "json",
    output: new TextEncoder().encode("body"),
    requesterIdHash: "aa".repeat(32), authChainSummary: "operator-self/aa…",
  });
  await emitReceipt(r_gen, "operator-bespoke", runtime);
  assert(r_gen.subKind === "report_generated", "report_generated built");

  const r_rej = buildReportRejected({ kind: "bespoke", reason: "test", requesterIdHash: "aa".repeat(32) });
  await emitReceipt(r_rej, "operator-bespoke", runtime);
  assert(r_rej.subKind === "report_rejected", "report_rejected built");

  const r_treg = buildTemplateRegistered({ templateHash: "01".repeat(32), operatorIdHash: "ab".repeat(32), schemaVersion: 1 });
  await emitReceipt(r_treg, "operator-bespoke", runtime);
  assert(r_treg.subKind === "template_registered", "template_registered built");

  const r_tsup = buildTemplateSuperseded({ oldTemplateHash: "01".repeat(32), newTemplateHash: "02".repeat(32), operatorIdHash: "ab".repeat(32) });
  await emitReceipt(r_tsup, "operator-bespoke", runtime);
  assert(r_tsup.subKind === "template_superseded", "template_superseded built");

  const r_ecb = buildErasureChainBroken({ cellIds: ["cd".repeat(32)], failingStep: "gc3", customerIdHash: "ef".repeat(32) });
  await emitReceipt(r_ecb, "operator-bespoke", runtime);
  assert(r_ecb.subKind === "erasure_chain_broken", "erasure_chain_broken built");

  const r_rl = buildRateLimitExceeded({ requesterIdHash: "aa".repeat(32), capLimit: 10, currentRate: 11 });
  ...
  const att = await generateRegistryAttestation(
    { kind: "registry-attestation", target: { agentIdHash: "aa".repeat(32) }, format: "json", auth: { path: "public" } },
    { registry, runtime },
  );
  ...
  try { new SaihmRuntimeClient("http://attacker.example.com/x", "Bearer t"); }
  catch (e) { httpsCheckThrew = (e as Error).message.includes("https"); }
  assert(httpsCheckThrew, "ctor rejects http:// non-localhost endpoint");

  let httpsLocalOk = true;
  try { new SaihmRuntimeClient("http://127.0.0.1:1234/x", "Bearer t"); }
  catch { httpsLocalOk = false; }
  assert(httpsLocalOk, "ctor allows http://127.0.0.1 (dev)");

  let httpsLoopOk = true;
  try { new SaihmRuntimeClient("http://localhost:1234/x", "Bearer t"); }
  catch { httpsLoopOk = false; }
  assert(httpsLoopOk, "ctor allows http://localhost (dev)");

  let badUrlThrew = false;
  try { new SaihmRuntimeClient("not a url", "Bearer t"); }
  catch (e) { badUrlThrew = (e as Error).message.includes("not a valid URL"); }
  ...
  try { await bigClient.recall(); }
  catch (e) { oversizeRejected = (e as Error).message.includes("response too large"); }
  bigServer.close();
...
main().catch((e) => { console.error("test crashed:", e); server.close(); process.exit(1); });
